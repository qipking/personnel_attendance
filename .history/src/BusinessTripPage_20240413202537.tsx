...
const heartRate = await client.request({
    method: 'GET',
    url: 'http://api.heclouds.com/devices/1052529105/datapoints?datastream_id=heart&start=2024-01-01T00:00:00&limit=20',
...
function BusinessTripPage() {
    ...
    for (let i = 0; i < temperature.data.data.length; i++) {
        HistoricalMotion.push({
            ...
            heartRate: heartRate.data.data[i].value,
            temperature: temperature.data.data[i].value,
            steps: steps.data.data[i].value
        });
    }
